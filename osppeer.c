...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	int j, last, avg, current;
	last = j = avg = current = 0;
	int samples[10];
	int sample_size = 10;

	for(j = 0; j < sample_size; j++)
		samples[j] = 10 * MIN_SPEED;
	...
	samples[current] = t->total_written - last;
	last = t->total_written;
	avg = 0;
...
static void task_upload(task_t *t)
{
	...
	struct stat data;
	if (curr_path == NULL || file_path == NULL ||
	    stat(file_path, &data) < 0 ||
	    strncmp(curr_path, file_path, strlen(curr_path))) {
		errno = ENOENT;
...
"         -dDIR        Upload and download files from directory DIR.\n"
	...
	pid_t pid_for_fork;
	int no_of_processes = 1;
	// First, download files named on command line.
	for (; argc > 1; argc--, argv++)
	  if ((t = start_download(tracker_task, argv[1])) && 
	      no_of_processes < MAX_PROCESSES)
	    {
	      if((pid_for_fork = fork()) < 0)
		error("Fork failed, skipping download of %s!\n", t->filename);
	      else if(!pid_for_fork)
		{
		  task_download(t, tracker_task);
		  exit(0);
		}
	  ...
	  else if (no_of_processes >= MAX_PROCESSES)
	    {
	      waitpid(-1, NULL, 0);
	      no_of_processes--;
	    }
	...
	while ((t = task_listen(listen_task)))
	  {
	    ...
	    while(no_of_processes > 1 && waitpid(-1, NULL, WNOHANG))
		  no_of_processes--;

	    if((pid_for_fork = fork()) < 0)
	      error("Fork failed, skipping upload of %s!\n", t->filename);
	    else if(!pid_for_fork)
	      {
		task_upload(t);
		exit(0);
